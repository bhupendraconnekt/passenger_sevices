...
export function getUserDataByParams(pageNo, size, params) {
  return {
...
// get station orders history
 export function getStationOrderHistory(page, limit, stationId){
  return async dispatch => {
    let link = `${API.OrderHistory}/${page}/${limit}?station_id=${"603498cf368b320cf0dc0a7d"}`;
    debugger
    let a = await dispatch(setIsLoading(true))
    axios({
      method: 'GET',
      ...
      if(response.data.success){
        debugger
        dispatch(setStationOrderHistory(response.data.order.docs, response.data.order.totalDocs, response.data.order.limit))
        // dispatch(setDashboardCount(response.data.counts))
      }
      dispatch(setIsLoading(false))
    }).catch(err => {
...
export function setStationOrderHistory(docs, total, limit) {
  return {
    type: "GET_ORDER_DOCS",
    docs: docs,
    total: total,
